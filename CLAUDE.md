...
- **Canonical values** (live in `shared-components.css`): `clamp(40px, 4vw, 52px)` / `500` / `#fff` / `1.15` / `-1px` / `margin: 0`, plus `≤768px: clamp(28px, 8vw, 40px)`. The 40px desktop floor is deliberate (2026-08): it equals the mobile cap so the size is continuous across the 768px breakpoint — the old 32px floor made headlines **shrink 8px as the viewport grew** past 768px (4vw doesn't reach 40px until 1000px; the 32px floor only ever bound at 769–800px). Production's `.section-headline` (use-case-page.css) still has the old discontinuity — fix via a `bolt-public-pages` PR, same one-line change. Margins are page layout — put them on wrappers or page-scoped rules (`.section-header .sc-section-h2 { … }`), never fork the base class for spacing.
...
- These are **going-forward standards (decision: Gary, 2026-08-04): they apply to the sandbox and to NEW pages. Existing live production pages keep their current spacing — do NOT open a "standardization" PR against them** (production microsoft's 42px hero is fine as-is). The once-planned "PR C" is cancelled, nothing is lost.
