...
try{ const b = localStorage.getItem('emailBanner_v3'); if (b) T.headerBanner = b; }catch(e){}
try{ Object.assign(T, JSON.parse(localStorage.getItem('emailTokens_v3')||'{}')); }catch(e){}
function saveT(){ try{
  const {logoSrc, footerImg, headerBanner, ...rest} = T;
  localStorage.setItem('emailTokens_v3', JSON.stringify(rest));
}catch(e){} }
...
function docFor(){
  ...
  if (t.socials === 'show') doc = doc.replace(
    ...
    `padding:0 ${t.cardPad}px 14px;text-align:left;">` +
    ...
    g.keys.forEach(d=>{
      ...
      if(d.type==='image'){
        ...
        clear.onclick=()=>{ T[d.k]=''; file.value=''; clear.style.display='none';
          if(d.k==='headerBanner'){ try{ localStorage.removeItem('emailBanner_v3'); }catch(e){} }
          paint(); };
...
function reset(){ T={...NOW};
  try{ localStorage.removeItem('emailTokens_v3'); localStorage.removeItem('emailBanner_v3'); }catch(e){}
  drawCtl(); paint(); }
function showSpec(){
